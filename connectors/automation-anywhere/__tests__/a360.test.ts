...
import { WorkflowService, ConnectorRegistry, InMemorySecretStore, queryCapability } from "@conduit/connector-sdk";
  ...
  const transport: HttpTransport = {
    async request(req: HttpRequest): Promise<HttpResponse> {
      ...
      if (path === "/v2/repository/file/list") {
        ...
        return jsonResponse(200, { list: opts.workflows });
      }
...
describe("Acceptance 1: token refresh on expiry, and refresh failure surfaces as a health failure", () => {
  it("refreshes an expired token without re-entry and keeps listing workflows", async () => {
    let clock = 1_000_000_000_000; // fixed start, ms
    const now = () => clock;
    const cr = fakeControlRoom({ baseUrl: "https://eu.cr", expectedApiKey: "KEY_EU", workflows: [validBot()], now, tokenLifetimeSeconds: 60 });
  ...
  it("reports a refresh failure as an unhealthy status and never crashes the listing path", async () => {
    ...
    const now = () => clock;
    const cr = fakeControlRoom({ baseUrl: "https://eu.cr", expectedApiKey: "KEY_EU", workflows: [validBot()], now });
    const secrets = new InMemorySecretStore({ "a360/eu": { username: "svc", apiKey: "KEY_EU" } });
    const c = new A360Connector("a360-eu", { controlRoomUrl: "https://eu.cr", secretRef: "a360/eu" }, secrets, { transport: cr.transport, now });
    ...
    // The workflow-listing path degrades via the service, not a crash: WorkflowService health-gates.
    const registry = new ConnectorRegistry();
    registry.register(c);
    const result = await new WorkflowService(registry, { error: () => {} }).list();
...
describe("Acceptance 2: A360 workflow payloads map to the domain model, with status normalisation", () => {
  it("maps ids/title/owner and folds A360 statuses onto WorkflowRunState, unknown -> Unknown", async () => {
    const now = () => 1_000_000_000_000;
    const workflows = [
      validBot({ id: 701, name: "Running Workflow", status: "RUNNING", createdBy: "Brad" }),
      validBot({ id: "702", name: "Done Workflow", status: "COMPLETED", createdBy: "Dana" }),
      validBot({ id: "703", name: "Off Workflow", status: "DISABLED", createdBy: "Priya" }),
      validBot({ id: "704", name: "Weird Workflow", status: "FROZEN_SOLID", createdBy: "Sam" }), // unknown status
    ];
    const cr = fakeControlRoom({ baseUrl: "https://eu.cr", expectedApiKey: "KEY_EU", workflows, now });
    const secrets = new InMemorySecretStore({ "a360/eu": { username: "svc", apiKey: "KEY_EU" } });
    const c = new A360Connector("a360-eu", { controlRoomUrl: "https://eu.cr", secretRef: "a360/eu" }, secrets, { transport: cr.transport, now });
    ...
    expect(result[0]).toMatchObject({ sourceId: "701", platform: "automation-anywhere", connectorId: "a360-eu", title: "Running Workflow", owner: "Brad" });
  });
});

describe("Acceptance 3: requesting an undeclared capability returns a clear unsupported result, not an exception", () => {
  it("reports workflows as supported and schedules as unsupported without throwing", async () => {
    const secrets = new InMemorySecretStore({ "a360/eu": { username: "svc", apiKey: "KEY_EU" } });
    const c = new A360Connector("a360-eu", { controlRoomUrl: "https://eu.cr", secretRef: "a360/eu" }, secrets);

    expect(queryCapability(c, Capability.Workflows)).toMatchObject({ supported: true });
...
describe("Acceptance 4: no stored secret value is reachable through any public surface", () => {
  it("keeps username/apiKey out of config, serialisation, describe(), and results", async () => {
    ...
    const now = () => 1_000_000_000_000;
    const cr = fakeControlRoom({ baseUrl: "https://eu.cr", expectedApiKey: SECRET_KEY, workflows: [validBot()], now });
    ...
    const surfaces = [JSON.stringify(c), JSON.stringify(await c.listWorkflows()), JSON.stringify(failHealth), JSON.stringify(await secrets.describe("a360/eu"))].join("\n");
...
describe("Acceptance 5: two Control Rooms return correctly attributed, non-mixed results", () => {
  it("keeps each instance's workflows attributed to its own connector, using its own credentials", async () => {
    const now = () => 1_000_000_000_000;
    const eu = fakeControlRoom({ baseUrl: "https://eu.cr", expectedApiKey: "KEY_EU", workflows: [validBot({ id: "1", name: "EU Workflow", createdBy: "Brad" })], now });
    const us = fakeControlRoom({ baseUrl: "https://us.cr", expectedApiKey: "KEY_US", workflows: [validBot({ id: "1", name: "US Workflow", createdBy: "Dana", status: "COMPLETED" })], now });
    ...
    expect(byConnector["a360-eu"]).toMatchObject({ id: "a360-eu:1", title: "EU Workflow", state: "Running" });
    expect(byConnector["a360-us"]).toMatchObject({ id: "a360-us:1", title: "US Workflow", state: "Idle" });
    ...
    const usOnly = await new WorkflowService(registry).list({ connectorId: "a360-us" });
